        ...
        override _.ExecuteAsync(stopToken: CancellationToken) =
            task {
                ...
                while not stopToken.IsCancellationRequested do
                    try
                        let! stats =
                            runOnce registry search record (fun () -> DateTimeOffset.UtcNow)
                        logger.LogInformation(
                            "tick: polled={polled} recorded={recorded} missing={missing} failed={failed}",
                            stats.Polled, stats.Recorded, stats.Missing, stats.Failed)
                    with
